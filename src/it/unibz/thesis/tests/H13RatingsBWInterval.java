...
public class H13RatingsBWInterval {
	...
	public static void main(String[] args) throws IOException {
		...
		MyLogger log = new MyLogger("bestworst-log-"+dataset);
		
		MyLogger results = new MyLogger("bestworst-res-"+dataset);
		...
		for (int i = 0; i < iterations; i++) {
			...
			List<Preference> group_bw = tools.highestAndLowestPreferences(unknown, unknown_predicted, ADD_ONCE);

			unknown.removeAll(group_bw);
			
			known.addAll(group_bw);
